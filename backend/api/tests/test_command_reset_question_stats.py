...
class CleanupQuestionStatsCommandTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        QuestionAnswerEvent.objects.create(question_id=cls.question_2.id, choice="a", source="question")
        QuestionFeedbackEvent.objects.create(question_id=cls.question_2.id, choice="like", source="quiz")
    ...
    def test_question_updates(self):
        ...
        management.call_command("reset_question_stats", self.question_1.id, "--no-input")
        ...
        self.assertEqual(
            QuestionAnswerEvent.objects.for_question(question_id=self.question_1.id).count(),
            0,
        )
        ...
        self.assertEqual(
            QuestionFeedbackEvent.objects.for_question(question_id=self.question_1.id).count(),
            0,
        )
        ...
        self.assertEqual(
            QuestionAnswerEvent.objects.for_question(question_id=self.question_2.id).count(),
            1,
        )
        self.assertEqual(
            QuestionFeedbackEvent.objects.for_question(question_id=self.question_2.id).count(),
            1,
        )
